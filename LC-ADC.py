...
def generate_synthetic_ecg(duration_s=3.0, fs=360, heart_rate=72):
	...
	duration_s: Duration of full ECG Signal
	...
	r_mask =		bool # Mask for R-part
	s_mask = 		bool # Mask for S-part
	t_mask = 		bool # Mask for T-part

	t = 			np.arange(0, duration_s, 1/fs)
	ecg = 			np.zeros_like(t)
	rr_interval = 	60.0 / heart_rate
	# --- ECG Signal Construction ---
	for beat_start in np.arange(0, duration_s, rr_interval):
		bt = t - beat_start # Calculate relative time
		
		# P wave  (~0.15 mV), from 0.05 to 0.25 seconds
		p_mask = (bt > 0.05) & (bt < 0.25)
		ecg[p_mask] += 0.15e-3 * np.exp(-((bt[p_mask]-0.15)**2)/(2*0.03**2))
		
		# Q wave  (~-0.10 mV), from 0.25 to 0.30 seconds
		q_mask = (bt > 0.25) & (bt < 0.30)
		ecg[q_mask] -= 0.10e-3 * np.exp(-((bt[q_mask]-0.27)**2)/(2*0.008**2))
		
		# R wave  (~1.2 mV), from 0.28 to 0.38 seconds. Overlaps with Q and S
		r_mask = (bt > 0.28) & (bt < 0.38)
		ecg[r_mask] += 1.20e-3 * np.exp(-((bt[r_mask]-0.33)**2)/(2*0.015**2))
		
		# S wave  (~-0.20 mV), from 0.35 to 0.42 seconds. Overlaps with R and T
		s_mask = (bt > 0.35) & (bt < 0.42)
		ecg[s_mask] -= 0.20e-3 * np.exp(-((bt[s_mask]-0.39)**2)/(2*0.010**2))
		
		# T wave  (~0.35 mV), from 0.40 to 0.65 seconds. Overlaps with S
		t_mask = (bt > 0.40) & (bt < 0.65)
		ecg[t_mask] += 0.35e-3 * np.exp(-((bt[t_mask]-0.52)**2)/(2*0.04**2))

	# Small Noise Addition to ECG Signal
	np.random.seed(42)
